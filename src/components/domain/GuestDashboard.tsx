  ...
  return (
    <div className="max-w-6xl mx-auto space-y-8">
      <ScrollReveal delay={0}>
        <section className="relative overflow-hidden rounded-[32px] p-8 lg:p-16 bg-gradient-to-br from-white via-primary-tint/20 to-reward-tint/20 border-2 border-line">
          <div className="absolute -top-20 -right-20 w-80 h-80 bg-primary rounded-full opacity-10 blur-3xl" />
          <div className="absolute -bottom-20 -left-20 w-96 h-96 bg-reward rounded-full opacity-10 blur-3xl" />
          <div className="absolute top-10 right-10 w-32 h-32 bg-rare rounded-full opacity-5 blur-2xl" />

          <div className="absolute top-6 right-6 lg:top-12 lg:right-12 hidden md:block">
      ...
      <ScrollReveal delay={100}>
        <Card className="p-5 bg-gradient-to-r from-primary-tint/30 to-reward-tint/30">
          <div className="grid grid-cols-2 md:grid-cols-4 gap-4 text-center">
            ...
            <div>
              <div className="font-display text-3xl font-extrabold text-success-dark">
                {totalGrammar}
              </div>
              <div className="text-xs font-bold uppercase tracking-widest text-ink-muted">
                Grammar topics
            ...
            <div>
              <div className="font-display text-3xl font-extrabold text-reward-dark">
                6
              </div>
              <div className="text-xs font-bold uppercase tracking-widest text-ink-muted">
                Games
            ...
            <div className="flex items-end justify-between mb-4">
              <div>
                ...
                <p className="mt-1 text-ink-soft text-sm">
                  New words just added to the library
                </p>
              </div>
              <Link
                href="/vocabulary"
                ...
                <Link key={w.slug} href={`/vocabulary/${w.slug}`}>
                  <Card interactive className="p-4 h-full">
                    <div className="flex gap-1.5 mb-2">
                      ...
                      {w.rarity !== "common" && (
                        <Badge
                          color={w.rarity === "epic" ? "rare" : "primary"}
                          size="sm"
                        >
                          {w.rarity}
                        </Badge>
            ...
            <Link href="/games/word-scramble">
              <Card interactive className="p-5 h-full">
                <div className="w-12 h-12 rounded-2xl bg-primary-soft flex items-center justify-center mb-3">
                  <Shuffle className="w-6 h-6 text-primary" strokeWidth={2.3} />
                </div>
                ...
                <div className="mt-3 text-xs font-bold text-primary">Play now →</div>
              </Card>
            </Link>
            <Link href="/games/match">
              <Card interactive className="p-5 h-full">
                <div className="w-12 h-12 rounded-2xl bg-success-soft flex items-center justify-center mb-3">
                  <Link2 className="w-6 h-6 text-success-dark" strokeWidth={2.3} />
                </div>
                <h3 className="font-display text-lg font-extrabold text-ink">
                  Word Match
                </h3>
            ...
            <Link href="/games/sentence-builder">
              <Card interactive className="p-5 h-full">
                <div className="w-12 h-12 rounded-2xl bg-reward-soft flex items-center justify-center mb-3">
                  <Trophy className="w-6 h-6 text-reward-dark" strokeWidth={2.3} />
                </div>
                <h3 className="font-display text-lg font-extrabold text-ink">
                  Sentence Builder
                </h3>
      ...
      <ScrollReveal delay={300}>
        <section>
          <div className="flex items-end justify-between mb-4">
            <div>
              <h2 className="font-display text-2xl lg:text-3xl font-extrabold text-ink flex items-center gap-2">
                <BookMarked className="w-6 h-6 text-primary" strokeWidth={2.3} />
                Browse by level
              </h2>
              <p className="mt-1 text-ink-soft text-sm">
            ...
            {CEFR_LEVELS.map((lv) => (
              <Link key={lv.level} href={`/vocabulary?level=${lv.level}`}>
                <Card interactive className="p-4 text-center">
                  ...
                  <div className="mt-2 text-xs font-bold text-ink-soft">{lv.label}</div>
                </Card>
              </Link>
            ))}
          </div>
          ...
          <Card className="p-8 lg:p-10 text-center relative overflow-hidden bg-gradient-to-br from-primary-tint/60 to-primary-soft/60 border-primary-tint">
            <div className="relative z-10">
              ...
              <p className="mt-2 text-ink-soft max-w-md mx-auto">
                Create a free account to save progress, track streaks, and unlock
